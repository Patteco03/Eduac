...
class Comprador_Model extends CI_Model {
	...
	public function RecuperaEmail($emailcomprador, $nomecomprador) {
		$this->db->select('codcomprador');
		$this->db->from('comprador');
		$this->db->where('emailcomprador', $emailcomprador, TRUE);
		$this->db->where('nomecomprador', $nomecomprador, TRUE);
		return $this->db->get()->result();
	}

		public function updateSenha($itens, $emailcomprador) {	
	     $this->db->where('emailcomprador', '"$emailcomprador"', FALSE);
	    $res = $this->db->update('comprador', $itens);
	    if ($res) {
