...
class ProdutoFoto_Model extends CI_Model {
	...
	public function getFoto($codproduto) {
		$this->db->select('produtofoto.codprodutofoto, produtofoto.produtofotoextensao, produtofoto.codproduto');
		$this->db->select('produto.nomeproduto ');
		$this->db->from('produtofoto');
		$this->db->join ('produto ','produto.codproduto = produtofoto.codproduto', 'INNER');
		$this->db->where("produtofoto.codproduto = $codproduto");
