...
if (! defined ( 'BASEPATH' ))
	...
	class Produto_Model extends CI_Model {
		...
		public function get($condicao = array(), $primeiraLinha = FALSE, $pagina = 0, $limite = LINHAS_PESQUISA_DASHBOARD, $ordenacao = FALSE, $tipoOrdem = "DESC") {
			...
			$this->db->select ( 'duracao, datainicio, datafinal, tipoproduto' );
			$this->db->where ( $condicao );
			$this->db->from ( 'produto' );
			$this->db->order_by ('codproduto', $tipoOrdem );
		...
		public  function  getCursoPorCategoria($codcategoria){
			$this->db->from ( 'produto' );
			$this->db->where ("codtipoatributo = $codcategoria" );
			
			return $this->db->get ()->result ();
		}
		
		public function getPorAutor($condicao = array()){
			$this->db->select('p.codproduto, p.nomeproduto, p.duracao, p.datainicio, p.datafinal, p.tipoproduto');
			$this->db->where($condicao);
			$this->db->from('produto p');
